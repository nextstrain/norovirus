...
DATASET_NAMES = config["nextclade_data"]["dataset_name"]

wildcard_constraints:
    DATASET_NAME = "|".join(DATASET_NAMES)

rule run_nextclade_genotype:
    input:
...
rule nextclade_metadata_genotype:
    ...
    params:
        nextclade_id_field=config["nextclade_data"]["id_field"],
        nextclade_field_map=lambda wildcard: [f"{old}={new}" for old, new in config["nextclade_data"][wildcard.DATASET_NAME]["field_map"].items()],
        nextclade_fields=lambda wildcard: ",".join(config["nextclade_data"][wildcard.DATASET_NAME]["field_map"].values()),
    shell:
        r"""
        exec &> >(tee {log:q})
...
rule join_metadata_and_nextclade:
    input:
        ...
        RdRp_metadata="results/RdRp/nextclade_metadata.tsv",
        VP1_metadata="results/VP1/nextclade_metadata.tsv",
    output:
    ...
    shell:
        ...
        augur merge \
            --metadata \
                ...
                rdrp={input.RdRp_metadata:q} \
                vp1={input.VP1_metadata:q} \
            --metadata-id-columns \
